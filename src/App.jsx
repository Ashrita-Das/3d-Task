...
function App() {
  ...
  useEffect(() => {
    if (showVideo && videoRef.current) {
      videoRef.current.pause(); // Pause video initially
    }
  }, [showVideo]);
      ...
      <AnimatePresence>
        {showVideo && (
          <motion.div
            ...
            transition={{ duration: 0.7, ease: "easeOut" }}
          >
            <video ref={videoRef} src={sampleVideo} muted={isMuted} className="video-player" />

            {!isFullscreen ? (
              <button className="play-button" onClick={enterFullscreen}>
